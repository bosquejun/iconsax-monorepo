
    import React from 'react'

    type IconPropTypes =  {
      size?: number;
      color?:string;
    }
    
    const FlashCircleOutlineIcon = (props:IconPropTypes) => {
      const { color = 'black', size = 24, ...otherProps } = props
      return (
        <svg width={size} height={size} viewBox="0 0 24 24" fill={color} {...otherProps} >
          <path d="M12 22.75C6.07 22.75 1.25 17.93 1.25 12C1.25 6.07 6.07 1.25 12 1.25C17.93 1.25 22.75 6.07 22.75 12C22.75 17.93 17.93 22.75 12 22.75ZM12 2.75C6.9 2.75 2.75 6.9 2.75 12C2.75 17.1 6.9 21.25 12 21.25C17.1 21.25 21.25 17.1 21.25 12C21.25 6.9 17.1 2.75 12 2.75Z" fill={color} />
<path d="M9.71003 17.84C9.42003 17.84 9.20005 17.7401 9.07005 17.6401C8.80005 17.4601 8.39004 17 8.65004 15.97L9.42006 12.86L8.50004 12.63C7.57004 12.4 7.36005 11.81 7.32005 11.57C7.28005 11.33 7.26005 10.7001 8.04005 10.1501L13.22 6.56003C14.09 5.96003 14.66 6.18002 14.93 6.36002C15.2 6.54002 15.61 7.00007 15.35 8.03007L14.58 11.1401L15.5 11.37C16.43 11.6 16.64 12.19 16.68 12.44C16.72 12.69 16.74 13.31 15.96 13.86L10.78 17.45C10.35 17.74 10 17.84 9.71003 17.84ZM9.11003 11.24L10.5101 11.5901C10.9101 11.6901 11.16 12.1 11.06 12.5L10.18 16.0401L14.8901 12.7701L13.49 12.4201C13.09 12.3201 12.84 11.91 12.94 11.51L13.8201 7.97001L9.11003 11.24Z" fill={color} />
        </svg>
      )
    }
    
    
    export default FlashCircleOutlineIcon;
      